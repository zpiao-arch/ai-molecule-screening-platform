...
- `apps/open-molecule-lab/`: prompt-first 本地工作台、MoleculeSet 校验、严格预检、确定性代码/阶段指纹、不可变 StageAttempt store、score/dock/report 编排、显式 resume、进程组取消/重启恢复、分页结果 API 和真实阶段时间线；不包含运行产物或 `node_modules`。
...
- `docs/`: 已确认的四级 CLI、Open Molecule Lab 和 verified stage resume 设计/实施规格。
